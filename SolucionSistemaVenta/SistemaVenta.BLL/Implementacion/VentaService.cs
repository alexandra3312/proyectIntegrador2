...
namespace SistemaVenta.BLL.Implementacion
{
    public class VentaService : IVentaService
    {
        ...
        {
            IQueryable<Producto> query = await _repositorioProducto.Consultar(p=> p.EsActivo == true && p.Stock > 0 && string.Concat(p.CodigoBarra, p.Marca, p.Descripcion).Contains(busqueda));
        ...
        public async Task<List<Venta>> Historial(string numeroVenta, string fechaInicio, string fechaFin)
        {
            ...
            if(fechaInicio!= "" && fechaFin != "")
            {
               ...
               return query.Where(v => v.FechaRegistro.Value.Date >= fech_inicio.Date &&
